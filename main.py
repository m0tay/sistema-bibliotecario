                    ...
                    match user_input:
                        case '1':
                            while True:
                                ...
                                pages = {'at': 1, 'total': 1}
                                pages['total'] = (len(books) + settings['page_size'] - 1) // settings['page_size'] # pyright: ignore
                                for idx, b in enumerate(books):
                                    if t.in_range(idx, page_range):
                                        print(f'{f'[{b.id}': >4}] {'Title:':.<24}{b.title}') # pyright: ignore
                                ...
                                match user_input:
                                    ...
                                    case '3':
                                        ...
                                        while not book and user_input != 'q':
                                            user_input = input('Select book (by id): ')
                                            t.clear_cli()

                                            book = db.read(Book, id=user_input)

                                            if not book:
                                                print('Maybe this book does not exist')
                                        ...
                                        match user_input:
                                            case '1':
                                                ...
                                                if book:
                                                    ...
                                                    title_input = get_non_empty_input(f'Title ({book.title}): ', default=book.title)
                                                    subtitle_input = get_non_empty_input(f'Subtitle ({book.sub_title}): ', optional=True, default=book.sub_title)
                                                    authors_input = get_non_empty_input(f'Authors ({book.authors}): ', default=book.authors)

                                                    while True:
                                                        try:
                                                            published_date_input = input(f'Published Date (YYYY-MM-DD) ({book.published_date}): ')
                                                            published_date = date.fromisoformat(published_date_input) if published_date_input else book.published_date
                                                            break
                                                        except ValueError:
                                                            print('Invalid date format. Using default date (2000-01-01).')
                                                            published_date = book.published_date if not published_date_input else date(2000, 1, 1)
                                                            break

                                                    synopsis_input = get_non_empty_input(f'Synopsis ({book.synopsis}): ', default=book.synopsis)
                                                    publisher_input = get_non_empty_input(f'Publisher ({book.publisher}): ', default=book.publisher)
                                                    isbn_input = get_non_empty_input(f'ISBN ({book.isbn}): ', optional=True, default=book.isbn)

                                                    genres_input = get_non_empty_input(f'Genres ({book.genres}): ', default=book.genres)
                                            ...
                                            case '2':
                                                ...
                                                match user_input:
                                                    ...
                                                    case 'q':
                                                        ...
                                                        break
                                                    case _:
                                                        print(f'{'Inexistent option':^{settings['width']}}')
                                    ...
                                    case '4':
                                        ...
                                        title_input = get_non_empty_input('Title: ')
                                        subtitle_input = get_non_empty_input('Subtitle (optional, press Enter to skip): ', optional=True)
                                        authors_input = get_non_empty_input('Authors: ')
                                    ...
                                    case '5':
                                        ...
                                        while not isinstance(user_input, int):
                                            try:
                                                user_input = int(input('Select user (by id) to permanently delete: '))
                                            except ValueError:
                                                t.clear_cli()
                                        ...
                                        match user_input:
                                            case 'delete':
                                                db.delete(Book, id=id_to_delete)
                                                print('Successfully deleted book')
                        ...
                        case '2':
                            while True:
                                ...
                                for idx, u in enumerate(users):
                                    if t.in_range(idx, page_range):
                                        print(f'[{u.id}] {'Name:':.<24}{u.name}') # pyright: ignore
...
def factory_users():
    ...
    for i in range(1, 41):
        db.add(User(
            ...
            register_date=date(randint(2020, 2025), randint(1, 12), randint(1, 28)),
        ), verbose=True) # pyright: ignore
...
def factory_books():
    ...
    for i in range(1, 41):
        db.add(Book(
            ...
            isbn=f'{randint(1000000000, 9999999999)}' if randint(0, 1) else None,
            genres=', '.join(choice(genres_list) for _ in range(randint(1, 3))),
